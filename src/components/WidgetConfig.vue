<template>
  <div v-if="show">
    <el-form label-position="top">
      <el-form-item label="标题" v-if="data.type!='grid' && data.type!='title'">
        <el-input v-model="data.name"></el-input>
      </el-form-item>
      <el-form-item label="内容" v-if="data.type=='title'">
        <el-input type="textarea" :rows="2" v-model="data.options.title"></el-input>
      </el-form-item>
      <el-form-item label="对齐方式" v-if="data.type=='title'">
        <el-radio-group v-model="data.options.alignment">
          <el-radio-button label="left">左对齐</el-radio-button>
          <el-radio-button label="right">右对齐</el-radio-button>
          <el-radio-button label="center">居中对齐</el-radio-button>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="字体大小" v-if="data.type=='title'">
        <el-input-number v-model="data.options.fontSize" :min="12" :max="32" :step="1"></el-input-number>
      </el-form-item>
      ...
      <el-form-item label="选项" v-if="Object.keys(data.options).indexOf('options')>=0">
        ...
        <template v-if="data.options.remote">
          <div>
            <el-input size="mini" style="" disabled="disabled" v-model="data.options.remoteFunc">
              <template slot="prepend">远端方法</template>
            </el-input>
            <el-input size="mini" style="border-color: red;" v-model="data.options.remoteApi" placeholder="使用Get请求方式的Api">
              ...
              <el-popover
                ...
                width="240"
                trigger="hover"
                content="Api接口请求方式必须为get，请求的数据格式为：{ data: { code: 200, list: [], msg: '成功' } }">
                <el-button slot="reference" :class="promptClass" :icon="promptIcon"></el-button>
              </el-popover>
            </el-input>
      ...
      <!-- <el-form-item label="选项绑定事件" v-if="Object.keys(data.options).indexOf('options')>=0">
        <ul>
          <li v-for="(item, index) in data.options.options" :key="index">
            <span style="font-size: 12px;">{{data.options.showLabel?item.label:item.value}}：</span>
            <el-select v-model="item.event" clearable style="margin-bottom: 5px;" placeholder="请选择绑定的事件">
              <el-option
      ...
      <el-form-item label="选项分析" v-if="Object.keys(data.options).indexOf('options')>=0">
        <ul>
          <li v-for="(item, index) in data.options.options" :key="index">
            <div style="font-size: 13px;">{{data.options.showLabel?item.label:item.value}}：</div>
            <el-checkbox class="lg-isAbnormal" style="margin-right: 15px;" size="mini" v-model="item.isAbnormal">异常</el-checkbox>
            <el-input-number size="mini" v-model="item.score" :step="1"></el-input-number>
            <el-input type="textarea" style="font-size: 12px;" :rows="2" v-model="item.introduce"></el-input>
          </li>
        </ul>
      </el-form-item>
      ...
      <el-form-item label="远端数据" v-if="data.type=='cascader'">
        <div>
          ...
          <el-input size="mini" style="" v-model="data.options.remoteApi" placeholder="使用Get请求方式的Api">
            <template slot="prepend">远端Api</template>
          </el-input>
          <el-input size="mini" style="" v-model="data.options.props.value">
            <template slot="prepend">值</template>
          </el-input>
      ...
      <template v-if="data.type != 'grid'">
        ...
        <el-form-item label="数据绑定Key">
          <el-input v-model="data.model" disabled="disabled"></el-input>
        </el-form-item>
...
export default {
  ...
  data () {
    return {
      // el-icon-success、el-icon-error、el-icon-question
      promptIcon: 'el-icon-question',
      promptClass: 'info',
      validator: {
        type: null,
        required: null,
    ...
    handleAddOption () {
      if (this.data.options.showLabel) {
        this.data.options.options.push({
          value: '新选项' + (this.data.options.options.length + 1),
          label: '新选项' + (this.data.options.options.length + 1),
          result: {
            score: 0,
            isAbnormal: false,
            introduce: ''
      ...
      axios.get(val)
        .then(res => {
          if (res.data.code === 200) {
            this.promptIcon = 'el-icon-success'
            this.promptClass = 'success'
            this.data.options.remoteOptions = res.data.list
            this.data.options.remoteResult = true
          } else {
            this.promptIcon = 'el-icon-error'
            this.promptClass = 'danger'
            this.data.options.promptResult = false
          }
        })
        .catch(err => {
          console.log(err)
          this.promptIcon = 'el-icon-error'
          this.promptClass = 'danger'
          this.data.options.remoteResult = false
        })
    }
  },
  watch: {
    ...
    'data.options.remoteApi': function(val) {
      if (val) {
        this.getfarEnd(val)
      } else {
        this.promptIcon = 'el-icon-question'
        this.promptClass = 'info'
      }
    },
  }
}
