<template>
  <el-form-item
    ...
    :label="element.name"
    :label-width="element.type == 'title' ? '0px' : ''"
  ...
  >
      ...
      >
        <template v-if="element.options.remote">
          <el-radio
            ...
            :label="item.value"
            v-for="(item, index) in element.options.remoteOptions"
    ...
    <template v-if="element.type=='title'">
      <div
        :style="{textAlign: element.options.alignment, fontSize: element.options.fontSize + 'px',lineHeight: element.options.fontSize + 12 + 'px'}"
        >{{element.options.title}}</div>
    </template>
...
export default {
  ...
  mounted() {
    if (this.element) {
      if (this.element.options.remote && this.element.options.remoteApi) {
        axios.get(this.element.options.remoteApi)
          .then(res => {
            if (res.data.code === 200) {
              this.element.options.remoteOptions = res.data.list
              this.element.options.remoteResult = true
            } else {
              this.element.options.promptResult = false
            }
          })
          .catch(err => {
            console.log(err)
            this.element.options.remoteResult = false
          })
      }
